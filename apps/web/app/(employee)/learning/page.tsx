...
function getRecommendedTopics(goalTypes: string[], completedIds: Set<string>, limit = 4): (Topic & { color: string; categoryLabel: string; categoryLabelDa: string })[] {
  ...
  const result: (Topic & { color: string; categoryLabel: string; categoryLabelDa: string })[] = []

  for (const cat of CURRICULUM) {
    if (!relevantCategoryIds.has(cat.id)) continue
    for (const topic of cat.topics.beginner) {
      if (!topic.href) continue
      if (completedIds.has(topic.id)) continue
      result.push({ ...topic, color: cat.color, categoryLabel: cat.label, categoryLabelDa: cat.labelDa })
      if (result.length >= limit) return result
    }
  }

  // Fallback: first incomplete topic from any category
  if (result.length === 0) {
    for (const cat of CURRICULUM) {
      for (const topic of cat.topics.beginner) {
        if (!topic.href || completedIds.has(topic.id)) continue
        result.push({ ...topic, color: cat.color, categoryLabel: cat.label, categoryLabelDa: cat.labelDa })
        if (result.length >= limit) return result
      }
    }
...
export default function LearningPage() {
  ...
  const recommended = getRecommendedTopics(goalTypes, completedIds, 4)
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-10 pb-20">
      ...
      >
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: '#6B5C52' }}>
            {en ? 'Learning hub' : 'Læringsplatform'}
          </p>
          <h1 className="text-2xl font-bold leading-snug" style={{ color: '#FFF8F3', fontFamily: 'Georgia, serif' }}>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium" style={{ color: '#9B8B7E' }}>
              {completedCount} / {totalLive} {en ? 'topics completed' : 'emner gennemført'}
            </p>
            <p className="text-sm font-bold" style={{ color: '#E8634A' }}>
              {pctDone}%
            </p>
          </div>
          <div className="h-2 rounded-full overflow-hidden" style={{ background: '#3D2B1F' }}>
            <div
              className="h-full rounded-full transition-all duration-700"
              style={{ width: `${pctDone}%`, background: '#E8634A' }}
            />
          </div>
        </div>
      </div>
      ...
      {lastTopic ? (
        <section>
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#9B8B7E' }}>
            {en ? 'Continue where you left off' : 'Fortsæt hvor du slap'}
          </p>
          <Link
            href={lastTopic.href}
            className="flex items-center gap-4 rounded-2xl p-5 transition-all hover:opacity-90 active:scale-[0.99]"
            style={{ background: '#FFF8F3', border: '1.5px solid #E8634A30' }}
          >
            <span className="text-3xl shrink-0">{lastTopic.icon}</span>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold mb-1 capitalize" style={{ color: '#9B8B7E' }}>
                {lastTopic.level}
              </p>
              <p className="text-sm font-semibold leading-snug" style={{ color: '#1E0F00' }}>
                {en ? lastTopic.question.en : lastTopic.question.da}
              </p>
            </div>
            <span
        ...
        /* First-time: nudge to start */
        <section>
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#9B8B7E' }}>
            {en ? 'Start here' : 'Start her'}
          </p>
          <Link
            href="/learning/practical/how-money-works"
            className="flex items-center gap-4 rounded-2xl p-5 transition-all hover:opacity-90"
            style={{ background: '#FFF8F3', border: '1.5px solid #E8634A30' }}
          >
            <span className="text-3xl shrink-0">💡</span>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold mb-1" style={{ color: '#9B8B7E' }}>
                {en ? 'Beginner · 5 min' : 'Begynder · 5 min'}
              </p>
              <p className="text-sm font-semibold leading-snug" style={{ color: '#1E0F00' }}>
                {en ? 'How money actually works — and why it matters to you' : 'Hvordan penge egentlig fungerer — og hvorfor det betyder noget for dig'}
              </p>
            </div>
            <span
              className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 text-sm"
      ...
      {hasGoals && recommended.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold uppercase tracking-widest" style={{ color: '#9B8B7E' }}>
              {en ? 'Recommended for your goals' : 'Anbefalet til dine mål'}
            </p>
            <Link
              href="/goals"
            ...
            {recommended.map((topic) => {
              ...
              return (
                <Link
                  ...
                  style={{
                    background: done ? '#F5F2EE' : '#fff',
                    border: `1px solid ${done ? '#C8C0B8' : '#EDE0D4'}`,
                  }}
                >
                  <span
                    className="w-2.5 h-2.5 rounded-full shrink-0"
                    style={{ background: done ? '#9B8B7E' : topic.color }}
                  />
                  <p
                    className="text-sm flex-1 leading-snug"
                    style={{ color: done ? '#9B8B7E' : '#1E0F00', textDecoration: done ? 'line-through' : 'none' }}
                  >
                    {en ? topic.label : topic.labelDa}
                  </p>
                  {done ? (
                    <span className="text-xs shrink-0" style={{ color: '#9B8B7E' }}>✓</span>
                  ) : (
                    <span className="text-xs shrink-0" style={{ color: '#9B8B7E' }}>→</span>
                  )}
                </Link>
          ...
          >
            {LEVELS.map((l) => (
              <button
                ...
                style={{
                  background: level === l.id ? '#FFF8F3' : 'transparent',
                  color: level === l.id ? '#1E0F00' : '#9B8B7E',
                }}
              >
                {en ? l.en : l.da}
              </button>
            ...
            return (
              <div key={cat.id}>
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  />
                  <h2 className="text-sm font-semibold" style={{ color: '#1E0F00' }}>
                    {en ? cat.label : cat.labelDa}
                  </h2>
                </div>
                  ...
                  {topics.map((topic) => {
                    ...
                    if (isLive) {
                      return (
                        <Link
                          key={topic.id}
                          href={topic.href!}
                          className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm transition-all hover:opacity-90"
                          style={{
                            background: done ? '#F0FBF5' : '#fff',
                            border: `1px solid ${done ? '#C8E6D2' : '#EDE0D4'}`,
                            color: done ? '#2D5C3E' : '#1E0F00',
                          }}
                        >
                          {done ? (
                            <span className="text-xs font-bold shrink-0" style={{ color: '#5B8A6B' }}>✓</span>
                          ) : (
                            <span
                          ...
                          <span className="flex-1 leading-snug">{en ? topic.label : topic.labelDa}</span>
                          <span className="text-xs shrink-0" style={{ color: done ? '#5B8A6B' : '#9B8B7E' }}>
                            {done ? 'Done' : '→'}
                          </span>
                        </Link>
                      )
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <span className="flex-1 leading-snug">{en ? topic.label : topic.labelDa}</span>
                        <span className="text-xs shrink-0" style={{ color: '#C8BDB5' }}>
                          {en ? 'Soon' : 'Snart'}
                        </span>
                      </div>
                    )
